...
- **`ModelGroupsPage.tsx` + `ModelGroupEditor.tsx`（新页，导航「接入 → 模型分组」）**：dnd-kit 拖拽排序列表（sensors / `SortableContext` / 乐观更新三件套照抄 `ModelsPage`），行内 `ModelGroupGlyph` + 名称 + 模型数 + 编辑/删除；删除走 `askConfirm` 并明确提示「组内 N 个模型会移到未分组，模型本身不会被删除」。编辑弹窗 = 实时预览行（与用户端同一个无底板图标组件，所见即所得）+ 名称 + `IconPicker` + 颜色（默认灰 + `COLOR_PRESETS` 预设 + react-colorful 自定义，三段式布局照抄 `TagsInput`）。
