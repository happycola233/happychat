...
- **近期里程碑 A–D（本文档已据此更新）**：
  ...
  - **J 模型分组 + 模型/分组图标**：管理员定义的全站分组（`model_groups`，拖拽排序，独立管理页），模型单一归属（`models.group_id`，删除分组只把成员移回未分组）；用户端模型选择器提供「平铺（分组标题可折叠）」与「二级目录（文件夹式钻取）」两种视图，账户级偏好记忆，模型数 >8 时出现搜索框。图标资源三来源统一为一个 JSON 字段（自托管 lobe-icons 内置库 / 管理员上传 / Emoji），未配置时按 modelId 自动识别品牌图标，管理端可批量套用识别结果；模型还可显式保存首字母模式以跳过自动识别。见 §4.2、§7.2、§9.1、§10。
...
- `ModelControlMenu`：**聚合选择器**（取代旧 ModelSelector + ChatControls）——一个触发器（模型名 + 思考深度短标签 + 联网状态小地球 + 箭头）聚合全部会话参数，桌面/移动共用同一套分区组件（`MenuSections`）：
  ...
  - **列对齐约定**（`ModelListViews.tsx` 顶部注释 + `TRAILING_SLOT_CLASS` / `OVERLAY_INFO_BUTTON_CLASS` 两个常量，对标管理端模型页的 `ROW_INSET_X` 系列度量）：模型行与分组成员统一 `pl-3`，不再用缩进表达层级；有图标的真实分组与模型共用左侧图标列，没有图标的「未分组」则直接从左侧内边距起排标题，不保留空图标槽。右侧统一 28px 操作列，ⓘ / 折叠箭头 / 钻取箭头共用同一中心线；有描述的模型把选中勾放在 ⓘ 左侧并为未选中态保留位置，无描述的模型则让选中勾直接复用最右侧 ⓘ 槽位。ⓘ 绝对定位覆盖在行上方（而非与行按钮并列），整行仍可点，`group-hover` 保证指针移到 ⓘ 上时行底色不闪断。相邻分区仅保留 4px 轻间隔；折叠箭头与组内内容以 200ms 同步过渡，内容用 CSS Grid `0fr ⇄ 1fr` 按真实高度展开，并在折叠态通过 `inert` 移出键盘交互。动画裁切使用 `overflow-clip` 而非 `overflow-hidden`，否则 Grid item 会丢失横向固有宽度贡献，令 `w-fit` 面板错误收窄并截断本可完整显示的模型名。
...
三种资源来源收敛为 `ModelIconAsset = {type:'lobe',slug} | {type:'custom',id} | {type:'emoji',char}`（`shared/types/domain.ts`）。模型字段使用 `ModelIcon = ModelIconAsset | {type:'initial'}`：`null` 表示未配置并允许自动识别，`initial` 表示显式强制名称首字母；分组只接受资源图标，不接受模型专用的首字母模式。

- **归一化**（`shared/util/modelIcon.ts`）：`normalizeModelIconAsset(value: unknown)` 只接受三种资源，`normalizeModelIcon(value: unknown)` 额外接受模型专用 `initial`；两者与 `normalizeModelTags` 契约一致——入参 `unknown`、绝不抛错、非法值静默降级为 `null`。slug 用 `LOBE_ICON_SLUG_PATTERN = /^[a-z0-9][a-z0-9-]{0,63}$/` 严格限制（**不允许点号**，挡住 `..` 穿越；不允许斜杠与标点，挡住 CSS/URL 注入），因为它会被拼进 `<img src>` 与 CSS `mask-image: url(...)`。Emoji 复用文件夹的单字素簇判定。zod 侧 `shared/schemas/model-group.ts` 拆成同口径的 `modelIconAssetSchema` / `modelIconSchema`，保证模型可写 `initial` 而分组明确拒绝；两端都在 DTO 边界调用（`toModelDTO`/`toModelGroupDTO`）。
