...
- **聚合模型选择器**：模型 / 思考深度 / 联网搜索 / X 搜索（图片模型则是分辨率 / 画质）收进一个菜单 —— 思考深度分段选择 + 一键固定默认，分辨率带宽高比缩略图；模型列表直接显示管理员配置且可自定义颜色的标签（如「内测」「禁止滥用」），带描述的模型有 ⓘ 气泡（桌面悬浮 / 移动端点按）；桌面端在输入框内弹出，移动端为底部弹层
...
| **模型** | **拖拽排序**、自定义标签文字与颜色、模型描述、**同 id 多实例**、手动添加、独立定价、请求体硬参数 JSON 覆写、思考等级拖拽排序 + 行内默认值、**按模型指定可用用户** |
