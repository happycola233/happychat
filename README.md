...
- **模型 / 分组图标**：内置 900+ AI 品牌图标（[lobe-icons](https://github.com/lobehub/lobe-icons)，**随应用自托管，不依赖公网 CDN**），也可上传自定义图标或直接用 Emoji；内置搜索支持中文品牌名、全拼与英文。未配置时按模型 ID **自动识别品牌图标**，管理端还能批量识别并套用；即使识别成功，也可显式改用名称首字母。单色图标经 CSS mask 渲染，浅色 / 深色下均自动适配
...
| **模型分组**            | 新建 / 重命名 / 删除 / **拖拽排序**；可选自定义图标，或为默认文件夹图形设置颜色；删除分组只把成员移回「未分组」，不会删除模型                                             |
