...
- `npm run typecheck` / `lint` / `test`（52 个单测文件、338 用例）/ `build` 全绿。
...
- `services/`：`models.ts`/`conversations.ts`/`folders.ts`/`admin.ts`/`providers.ts`（`syncProviderModels` 供后台同步模型使用）—— DB 查询 + DTO 映射，路由层只编排不写复杂 SQL。`conversations.ts` 的 `deleteConversations`（单条删除与批量删除共用，级联消息/runs 并清理附件行+磁盘文件）、`moveConversationsToFolder`（逐行保留 `updatedAt`，避免移动打乱最近排序）。
- `routes/`：`auth`/`admin`/`models`/`conversations`（含 `POST /batch-delete`、`POST /batch-move`）/`folders`（文件夹 CRUD + 置顶）/`emoji-data`（自托管 Emojibase 数据，公开读）/`runs`/`attachments`/`shares`/`announcements`，见各节。
...
- 单测（`npm run test`）：覆盖 URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、run 准备、前端流式 reducer 与推理摘要 Markdown（含多 reasoning item/summary part、续传压缩、终态校准及历史相邻粗体兜底）、Markdown CJK 邻接强调/安全性（危险 HTML/协议过滤——公告正文复用该渲染器）与 GitHub Alerts 渲染。
