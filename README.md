...
- **标题总结也照实记账**：会话标题的后台调用同样计费、同样占额度，后台「请求事件」可按请求类型单独过滤或隐藏
...
| **用户限额**            | 策略模板 CRUD / 复制 / 设为默认 / 拖拽排序；用户视图查看每人策略与用量进度、批量修改策略、暂停或恢复限额、按桶或整体重置周期、临时赠送额度、逐规则覆写（含优先级）并在保存前**预览最终生效结果** |
