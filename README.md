...
- **多模态输入输出**：图片输入、文件输入（OpenAI 文件格式；Anthropic 原生图片、PDF 与纯文本 document block）、图片生成（GPT-Image-2，支持分辨率与画质选择）。等待时以正方形点阵中的蓝紫流光展示生成状态与真实耗时，保留上游提供的 partial image 中间预览，新图加载完成后平滑过渡，支持多图进展与放大查看；等待时可展开贪吃蛇，用键盘、方向按钮或触屏滑动操作，切到后台自动暂停。Images API 非流式请求仍在完成后显示最终图片。
