...
- **导出聊天**：六种格式一键导出——chatlog-md 对话日记（遵循 [chatlog-md/1 规范](https://github.com/happycola233/dialogary/blob/main/chatlog-md-%E6%A0%BC%E5%BC%8F%E8%A7%84%E8%8C%83.md)，适合长期保存与程序解析）、Markdown、自包含 HTML 网页（附件内联、双主题）、JSON 全量数据（可含完整分支树）、JSONL（OpenAI messages，适合微调数据集）、纯文本；支持消息逐条挑选或快捷选择全部消息 / 全部用户消息 / 全部 AI 回复，思考过程 / 模型名 / 引用来源 / 检索过程 / Token 用量逐项开关，时间精度四档，附件三种模式（打包 ZIP + assets · 仅文件名 · 不包含）；JSON 结构化导出以有序 `processSteps` 统一表达思考、进展说明和检索动作（不再输出旧 `reasoningSummary` / `searchActions`）；弹窗内实时预览导出效果；侧栏批量模式可多选一键批量导出
