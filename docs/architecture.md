...
- **近期里程碑 A–D（本文档已据此更新）**：
  ...
  - **H 导出聊天**：六种格式（chatlog-md 对话日记 / Markdown / 自包含 HTML / JSON 全量 / JSONL messages / 纯文本）× 多维选项（消息范围与逐条选择、思考摘要、模型名、引用来源、联网搜索过程、Token 用量、时间精度四档、附件三模式），弹窗实时预览，批量导出打包 ZIP（JSONL 合并单文件）。见 §3.3 `export/` 与 §7.2。
- 验证方式：`scripts/` 下脚本（Playwright 浏览器 E2E + 直连后端冒烟）+ 临时库 curl 冒烟，全部对真实 OpenAI 兼容上游（Base URL/Key 在管理后台 Provider 中配置，不写进代码）跑通。
- `npm run typecheck` / `lint` / `test`（80 个单测文件、528 用例）/ `build` 全绿。
...
- `export/`：**聊天导出**。`collect.ts`（归属校验 + 当前分支/全树范围 + 逐条选择过滤 + 附件行解析与读盘降级）→ 六个格式构建器：`chatlog-md.ts`（严格遵循 dialogary [《chatlog-md 格式规范 v1》](https://github.com/happycola233/dialogary/blob/main/chatlog-md-%E6%A0%BC%E5%BC%8F%E8%A7%84%E8%8C%83.md)——行首哨兵、front matter、日期节、`> 🤔` 思考块、`🖼️/📄` 附件行、`<!-- @meta -->` 扩展点、§10 反斜杠转义；转义只针对**严格匹配哨兵正则**的行，保证往返保真）、`markdown.ts`、`text.ts`、`json.ts`（happychat-export/1，full 范围含整棵分支树 + activeLeafId）、`jsonl.ts`（OpenAI messages，一行一会话）、`html.ts`（unified + rehype-raw/sanitize 服务端渲染 Markdown，附件内联 data URI，浅/深主题自适应单文件）；`index.ts` 编排（fflate ZIP 打包 `assets/`、预览模式不读盘但用 existsSync 保证展示一致、批量导出每会话独立文件夹）。选项 zod schema 在 `shared/schemas/export.ts`；**格式能力矩阵 + 归一化**在 `shared/util/exportOptions.ts`（前端据此禁用控件、服务端据此在构建前钳制选项，两端共用同一份事实）。
...
- `routes/`：`auth`/`admin`/`models`/`conversations`（含 `POST /batch-delete`、`POST /batch-move`、`POST /export-batch`、`POST /:id/export`（preview=true 返回 JSON 预览，否则文件流 + RFC 5987 中文文件名）、`POST /:id/branch`）/`folders`（文件夹 CRUD + 置顶）/`emoji-data`（自托管 Emojibase 数据，公开读）/`runs`/`attachments`/`shares`/`announcements`，见各节。
...
- 前端不消费上游 `response.completed` 的内容终值（`output_item.added/done` 仅用于读取 `web_search_call` 的生命周期与动作，不作为正文/思考来源）；本地合成的 `run.done` 是浏览器权威终值（含 `webSearchActions` 终态校准），因此服务端保留终态校准、同时从所有客户端事件中剥离重复 opaque 密文不会改变现有 UI 路径。
...
- **web_search 动作通用解析（`shared/util/webSearchActivity.ts`）**：上游把「搜索、打开页面、页内查找」都记为一次 `web_search_call`；lifecycle 事件（`in_progress/searching/completed`）只报生命周期，**查询词等动作细节要等 `output_item.done`/终态 `response.output` 才出现**（实测 completed→done 相隔 0~117ms），因此「正在搜索」阶段拿不到当前搜索词。解析优先级 `action.queries[]` → `action.query`（官方已废弃仍兼容）→ `JSON.parse(arguments)` → `JSON.parse(input)`（后两者兼容 xAI Responses 旧实现：item 首次出现即 completed、无专用 lifecycle 事件、id 为 `fc_` 前缀——一律不判断 id 前缀）。`engine.ts` 在 `output_item.added/done` 与终态 output 三处收口（Map 保序去重），经 `finalize.ts` 写 `messages.web_search_actions` 并随 `run.done.webSearchActions` 下发终态权威值；前端 reducer 用同一解析器，两端口径一致。
...
- `ReasoningCard`：组件内计时器（每 200ms）+ 思考中自动展开/完成自动折叠 + 自动滚底。**流式动画**：思考中（`status='thinking'`）摘要体给 `Markdown` 传 `animate`，复用正文的 `hc-stream-seg` 逐单元渐入；新小节整体以 `hc-reasoning-step-in` 低位淡入——与联网搜索卡的 `hc-websearch-step-in` 共享 `hc-step-in` 关键帧（同时长/曲线），两卡运动语言统一；小标题在闭合 `**` 时整行一次性到达，`AnimatedSectionTitle` 按可见单元（`splitVisibleUnits`，CJK 逐字/ASCII 整词）注入逐单元 delay 做从左到右扫入（24ms 步长、360ms 封顶，呼应搜索词 chips 的错峰），圆点以 `hc-reasoning-dot-in` 同步弹现，单元按位置 key 保持身份避免正文流入时重播；完成页脚同样走 `hc-reasoning-step-in` 入场；完成/持久化态静态渲染不重播，动画均登记 `prefers-reduced-motion` 关停。⚠ 状态行与摘要体之间的间距放在可折叠容器内部（`pt-2`）而**不是根上的 space-y**：Tailwind v4 的 space-y 给前一个兄弟加 margin-bottom，折叠后 0 高度容器会留下死空间，导致「已思考」与下方搜索状态行折叠时间距偏大（已按用户反馈修正，勿改回）。
- `WebSearchActivity`：**联网搜索状态卡**（思考卡下方、正文上方；分享页复用）。数据来自 reducer 的 `LiveMessage.webSearchCalls`（`{id,status,action}[]`，`persistedWebSearchCalls()` 把持久化 `MessageDTO.webSearchActions` 适配成同一形状）。状态行：进行中为呼吸地球 + 流光「正在搜索网页」（复用 `hc-reasoning-shimmer`，变量在 `.hc-websearch` 上重声明），完成后按 `summarizeWebSearchActions` 显示「已搜索 N 个关键词 · 浏览 M 个页面」（页面按 URL 去重）。明细为按时间序的动作行：搜索词 chips 逐个错峰渐入（`hc-websearch-chip-in`）、进行中调用先渲染流动骨架 pill（查询词要等调用完成才回传，不猜内容）、动作到达时换 key 原位渐入；页面行显示「主机名+路径」并外链。自动展开/折叠：搜索开始自动展开、正文开始输出或终态自动折叠，用户手动开合后自动逻辑让位；动画全部登记进 `prefers-reduced-motion` 关停名单。终态由 `run.done.webSearchActions` 校准，与流式解析一致时保留行身份避免重播入场动画；未解析出动作的占位调用在终态被丢弃，与刷新后读库口径一致。
- `buildPath.ts`：客户端从 `activeLeafId` 沿 `parent_id` 构建可见路径 + `getSiblings`（与服务端 `services/conversations.ts` 的 `buildPath`/`deepestLeaf` 对应）。
- **导出聊天**（`ExportDialog.tsx`）：三个入口——顶栏三点菜单、侧栏行菜单（id 冒泡 + 顶层单实例）、批量工具栏「导出」（`batch-export`，完成后自动退出批量模式）。弹窗 = 格式卡片（六种，`EXPORT_FORMAT_CAPS` 提供文案）+ 选项区（能力矩阵联动：不适用的开关置灰并注明「当前格式不支持」）+ 消息逐条选择（单会话且当前分支时；默认全选，`selected=null` 表示全部）+ 350ms 防抖实时预览（`preview:true`，`placeholderData` 保留上一帧防闪烁）。下载走 `api/client.ts apiPostFile`（blob + 解析 `filename*`）→ `api/export.ts saveBlobToFile`。
...
- 后端：`routes/admin.ts`（全 `requireAdmin`）+ 服务层 `services/stats.ts`（概览/分析/分用户/请求事件/错误事件，drizzle `sql` 求和分组 + 整除时间分桶 + 成本经 `shared/util/cost.ts costUsd`）、`services/sessions.ts`（会话）、`services/appConfig.ts`（注册邀请码要求/分享/标题总结等全局设置）、`services/shares.ts`（分享）、`services/admin.ts`（邀请码/用户/`getStats` 系统信息）、`services/models.ts`（含 `createModel` 手动添加，**不再查重**——同 id 多实例合法）、`services/providers.ts`（`syncProviderModels` 一键同步 + `getProviderModelCatalog`/`importProviderModels` 上游目录挑选添加，路由 `GET /providers/:id/catalog`、`POST /providers/:id/import-models`）、`services/announcements.ts`（站内公告 CRUD + 读者名单 + 重置已读）。
...
- `ProvidersPage.tsx`：卡片含元信息定义列（Base URL/Key）与底部操作栏（测试连接/**挑选模型**/同步全部/编辑/删除右置）；「挑选模型」弹窗拉上游目录、搜索勾选后批量添加，已存在的 id 标注实例数、再勾选即再建一个实例。`SettingsPage.tsx`：新增“注册”卡片，“注册需要邀请码”与分享开关均为**切换即存**；注册要求默认开启，常驻说明明确首位管理员始终免邀请码，关闭时另以 amber 文案警告开放注册的批量注册与资源滥用风险；标题提示词以**已填写状态**呈现（内置默认在 `shared/constants.ts DEFAULT_TITLE_PROMPT`，与默认一致时存 null），带「恢复默认」。账号中心的“邀请码”页改为中性说明并链接到系统设置；其余子页仍为页头下方的下划线标签（用户/邀请码/会话）。`SharesPage.tsx` 列出全部分享并可撤销。

---

## 11. 测试与验证（`scripts/` + vitest）

- 单测（`npm run test`）：覆盖全局注册配置（新部署默认开启、切换持久化、其他字段的局部更新不重置）与注册策略（首位管理员始终免邀请码、默认开启时缺失/空白邀请码均拒绝、关闭后开放注册且不消耗残留邀请码、重新开启后恢复校验与兑换、`bootstrap` 状态同步）、URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、模型按用户授权（默认全员、管理员不绕过、全局开关优先、未知用户原子拒绝、跨批写入、删除最后一位授权用户不意外开放、前端搜索/分组/选择保持）、生产静态缓存（HTML/SPA 回退重新验证、哈希资源 GET/HEAD/Range/304 长缓存、缺失旧资源 404、API 路由隔离）、run 准备与越权模型拒绝、历史 reasoning 的事件净化/历史清理幂等/终态提存/256KB 门控/来源与版本门控/原样顺序/DTO 分享隔离/分支复制/流前单次降级与错误密文脱敏/无终态 EOF 失败、未发送附件 24h 边界/历史单会话修复与跨会话保护/千条以上续批/前批失败不阻塞且可重试/越界路径保护/调度恢复与防重入、独立分支的路径截断/权限/父链/参数恢复/计时快照/附件独立生命周期与失败回滚、登录/注册的密码强度评分档位、前端分支按钮/原始图标/持久化流式态门控、流式 reducer 与推理摘要 Markdown（含多 reasoning item/summary part、续传压缩、终态校准及历史相邻粗体兜底）、Markdown CJK 邻接强调/安全性（危险 HTML/协议过滤——公告正文复用该渲染器）与 GitHub Alerts 渲染、分享（选择链校验 `shareSelection`、附件剥离、'keep' 有效期、撤销后换新 token、过期失效）、导出（chatlog-md 规范符合与严格哨兵转义、六格式产物、附件缺失降级、预览一致性、批量 ZIP/JSONL、归属校验、流式占位剔除、选项能力矩阵归一化）。
...
| 联网搜索状态（动作解析/持久化/UI）                               | `shared/util/webSearchActivity.ts`、`server/runs/engine.ts` + `finalize.ts`、`web/src/sse/eventReducer.ts`、`web/src/chat/WebSearchActivity.tsx` + `index.css` 的 `.hc-websearch-*`                                   |
