...
- **检索动作通用解析（`shared/util/searchActivity.ts`）**：两类服务端检索工具收敛到同一个 `SearchAction[]` 有序数组。① **web_search**：上游把「搜索、打开页面、页内查找」都记为一次 `web_search_call`；lifecycle 事件（`in_progress/searching/completed`）只报生命周期，**查询词等动作细节要等 `output_item.done`/终态 `response.output` 才出现**（实测 completed→done 相隔 0~117ms），因此「正在搜索」阶段拿不到当前搜索词。解析优先级 `action.queries[]` → `action.query`（官方已废弃仍兼容）→ `JSON.parse(arguments)` → `JSON.parse(input)`（后两者兼容 xAI Responses 旧实现：item 首次出现即 completed、无专用 lifecycle 事件、id 为 `fc_` 前缀——一律不判断 id 前缀）。② **x_search**（xAI Grok 检索 X 站内内容）：上游把它拆成若干 server-side 子工具，在 Responses 协议里表现为 `custom_tool_call`（实测 `x_keyword_search` / `x_semantic_search` / `x_user_search` / `x_thread_fetch`；**没有** `x_search_call` item，也**没有**专用 lifecycle 事件）——`item.name` 即动作类型，参数是 JSON 字符串 `item.input`（`query`/`mode`/`usernames`/`excluded_usernames`/`from_date`/`to_date`/`post_id`），并由独立的 `response.custom_tool_call_input.delta|done` 下发，**`input.done` 比 `output_item.done` 早到**，因此 UI 能在调用结束前就显示查询词。未知的 `x_*` 子工具凭 `call_id` 的 `xs_` 前缀归类为通用 `x_search`，业务自定义工具（无 `xs_` 前缀）一律不认，避免误判。同一次调用会被多次上报（`added` 常常只有类型），`mergeSearchAction` 只接受**信息量不减少**的覆盖。`engine.ts` 在 `output_item.added/done`、`custom_tool_call_input.done` 与终态 output 四处收口（Map 保序去重），经 `finalize.ts` 写 `messages.search_actions` 并随 `run.done.searchActions` 下发终态权威值；前端 reducer 用同一解析器，两端口径一致。
- `params.ts` `buildResponseBody`：参数优先级 **高级硬参数 > 应用生成参数 > 用户请求 > 模型默认 > 代码默认**。思考按 `allowed_efforts` 校验后才发 `reasoning.effort`；历史推理开关 + 有效非 `none` effort 时加 `include:['reasoning.encrypted_content']`，`include` 与高级参数按去重并集合并（关开关不删除管理员手写 include）；联网仅 `capabilities.web_search && 开关` 才挂 `tools:[{type:'web_search'}]`，X 搜索同理挂 `{type:'x_search'}`（两者独立、可同时下发，`responseToolMergeKey` 各自成键以便高级 JSON 覆盖）；开思考时 `max_output_tokens` 兜底 ≥25k（`REASONING_MIN_OUTPUT_TOKENS`）。所有文本请求写入会话级稳定 `prompt_cache_key`；最后 `mergeDeep(hardParams)`，因此高级 JSON 可显式覆盖该 key 并原样传递其他上游参数。chat/completions 同样处理。`buildImageBody`/`buildImageEditBody` 只组装 size/quality/background 与参考图；`gpt-image-2` 尺寸规则在 `prepare.ts` 调共享 util 校验，避免把 provider fetch 层写成业务校验层。
...
13. **chat/completions 为子集**：`model.kind='chat'` 支持文本/思考(`reasoning_content`)/图片输入，但**不支持文件输入与 web_search / x_search 等服务端工具**（这些仅 Responses 路径）。
