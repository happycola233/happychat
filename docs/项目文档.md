...
- `npm run typecheck` / `lint` / `test`（62 个单测文件、388 用例）/ `build` 全绿。
...
- `ModelEditor.tsx` **新建/编辑两用**：模型 ID、外显名称、**描述（用户可见，选择器 ⓘ）**、**标签（`TagsInput.tsx` 芯片输入，与用户端同配色所见即所得）**、能力（两列开关）、思考等级（`ReasoningEffortEditor.tsx`：**单容器行内编辑列表 + 拖拽排序 + 行内「设默认」标记**，幽灵输入框；不再有单独的默认下拉框）、默认参数、**定价（USD/1M，分别配置普通输入、缓存写入、「缓存读取/输入」、输出）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON）**。重复的“在用户端启用”开关已移除，避免旧表单覆盖列表里的最新全局状态。
...
- 单测（`npm run test`）：覆盖 URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、模型按用户授权（默认全员、管理员不绕过、全局开关优先、未知用户原子拒绝、跨批写入、删除最后一位授权用户不意外开放、前端搜索/分组/选择保持）、run 准备与越权模型拒绝、未发送附件 24h 边界/历史单会话修复与跨会话保护/千条以上续批/前批失败不阻塞且可重试/越界路径保护/调度恢复与防重入、独立分支的路径截断/权限/父链/参数恢复/附件独立生命周期与失败回滚、前端分支按钮/原始图标/持久化流式态门控、流式 reducer 与推理摘要 Markdown（含多 reasoning item/summary part、续传压缩、终态校准及历史相邻粗体兜底）、Markdown CJK 邻接强调/安全性（危险 HTML/协议过滤——公告正文复用该渲染器）与 GitHub Alerts 渲染。
