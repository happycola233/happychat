...
- **侧边栏文件夹 + 批量管理（里程碑 E）**：
  ...
  - **文件夹设置弹窗**（`FolderEditorDialog.tsx`，创建/编辑两用，全局挂载由 `store/folderEditor.ts` 驱动）：`FolderIdentityField.tsx` 让 44px 高的名称输入与图标触发器并排；触发器内仍是 24px 无底板裸图标，仅用同样无底板的 ChevronDown 表达展开状态，不再向 Emoji 叠加装饰角标，图形尺寸与点击热区各自独立。桌面端新建时自动聚焦名称、展开 Emoji 时自动聚焦搜索，移动端两处均不抢焦点（避免连续唤起软键盘）；Emoji 面板 = **frimousse**（headless，`EmojiPickerPanel.tsx` 懒加载、Tailwind 全自绘、`locale=zh` 中文搜索、数据走同源 `/api/emoji-data`——服务端自托管 Emojibase，不依赖公网 CDN）；颜色 = 默认灰 + 12 色柔和浅色色板（与模型分组共用，含 Windows 文件夹黄 `#ffdd7c`）+ 自定义（入口使用同系粉彩渐变；**react-colorful** HexColorPicker/HexColorInput，`.hc-color-picker` 覆写样式）；面板均内联展开，Emoji 列表按弹窗剩余高度自适应并独立滚动（浏览器放大时不产生弹窗外层滚动条），Escape 先收面板再关弹窗。
...
- **`ModelGroupsPage.tsx` + `ModelGroupEditor.tsx`（新页，导航「接入 → 模型分组」）**：dnd-kit 拖拽排序列表（sensors / `SortableContext` / 乐观更新三件套照抄 `ModelsPage`），行内 `ModelGroupGlyph` + 名称 + 模型数 + 编辑/删除；删除走 `askConfirm` 并明确提示「组内 N 个模型会移到未分组，模型本身不会被删除」。编辑弹窗 = 实时预览行（与用户端同一个无底板图标组件，所见即所得）+ 名称 + `IconPicker` + 默认文件夹颜色（默认灰 + 与聊天文件夹共用的 `FOLDER_COLOR_PRESETS` 柔和浅色预设 + react-colorful 自定义，入口同样使用粉彩渐变）。显式图标与颜色互斥：选择图标即清空并隐藏颜色字段；三个色板的 28px 无边框圆点共用 `components/ui/ColorSwatch.tsx`，避免 `conic-gradient` 与半透明边框再次产生四边串色。
