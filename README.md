...
- **聚合模型选择器**：模型 / 推理强度 / 联网搜索 / X 搜索（图片模型则是分辨率 / 画质）收进一个菜单 —— 推理强度分段选择，再点已选档位可回到自动，也可一键固定默认；分辨率带宽高比缩略图；模型列表直接显示品牌图标与管理员配置且可自定义颜色的标签（如「内测」「禁止滥用」），带描述的模型有 ⓘ 气泡（桌面悬浮 / 移动端点按）；桌面端在输入框内弹出，移动端为底部弹层
