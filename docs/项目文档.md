...
- `messages`（**合并了节点树与内容，单表**）：`parent_id` 自引用构成分支树（无 DB 外键）；`content` 是 `ContentPart[]`；用户消息的 `runtime_context` 冻结发送时的日期时间/时区，仅构建上游请求时展开为隐藏 system 消息；另有 `run_id`、`reasoning_summary`、`reasoning_duration_ms`/`generation_duration_ms`（展示快照，run 仍在时读取期现算值优先）、`annotations`、各 token 列、`error_message`。索引 `(conversation_id, parent_id)` 支撑树遍历。
...
- `ModelEditor.tsx` **新建/编辑两用**：模型 ID、外显名称、**描述（用户可见，选择器 ⓘ）**、**标签（`TagsInput.tsx` 芯片输入，与用户端同配色所见即所得）**、能力（两列开关）、思考等级（`ReasoningEffortEditor.tsx`：仅在思考能力开启时显示，关闭后隐藏但保留配置；**单容器行内编辑列表 + 拖拽排序 + 行内「设默认」标记**，幽灵输入框；手动新建时预填 `none/low/medium/high/xhigh` 五档但不擅自设置默认档位，不再有单独的默认下拉框）、默认参数、**定价（USD/1M，分别配置普通输入、缓存写入、「缓存读取/输入」、输出）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON）**。重复的“在用户端启用”开关已移除，避免旧表单覆盖列表里的最新全局状态。
