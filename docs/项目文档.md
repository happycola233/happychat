...
- `npm run typecheck` / `lint` / `test`（60 个单测文件、370 用例）/ `build` 全绿。
...
- `ModelEditor.tsx` **新建/编辑两用**：模型 ID、外显名称、**描述（用户可见，选择器 ⓘ）**、**标签（`TagsInput.tsx` 芯片输入，与用户端同配色所见即所得）**、能力（两列开关）、思考等级（`ReasoningEffortEditor.tsx`：**单容器行内编辑列表 + 拖拽排序 + 行内「设默认」标记**，幽灵输入框；不再有单独的默认下拉框）、默认参数、**定价（USD/1M，分别配置普通输入、缓存写入、「缓存读取/输入」、输出）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON）**。
