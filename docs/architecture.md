...
- **近期里程碑 A–D（本文档已据此更新）**：
  ...
  - **J 模型分组 + 模型/分组图标**：管理员定义的全站分组（`model_groups`，拖拽排序，独立管理页），模型单一归属（`models.group_id`，删除分组只把成员移回未分组）；用户端模型选择器提供「平铺（分组标题可折叠）」与「二级目录（文件夹式钻取）」两种视图，账户级偏好记忆，模型数 >8 时出现搜索框。图标资源三来源统一为一个 JSON 字段（自托管 lobe-icons 内置库 / 管理员上传 / Emoji）；分组还可显式保存 `none`，像「未分组」一样完全移除图标槽。未配置模型图标时按 modelId 自动识别品牌，管理端可批量套用识别结果；模型还可显式保存首字母模式以跳过自动识别。见 §4.2、§7.2、§9.1、§10。
...
- `model_groups`：**模型分组**（管理员定义的全站结构，非用户私有）——`name`/`icon`(JSON `ModelGroupIcon`：null=默认文件夹图形，`{type:'none'}`=完全不显示图标，其余为 `ModelIconAsset`)/`color`(#RRGGBB，默认文件夹图形为空时归一为默认黄色)/`sort`。`icon` 与 `color` 在产品语义上互斥：只有默认文件夹图形使用自定义颜色，显式资源图标使用自身外观，无图标模式也固定清空颜色；`shared/util/modelGroupAppearance.ts resolveModelGroupColor` 被服务端写入/DTO 与前端预览/渲染共同调用，旧的 `icon+color` 脏组合也不会在移除图标后让隐藏颜色突然复活。新增状态只扩展 JSON 联合类型，无需 DDL，既有 null 数据语义不变。排序沿用 **models 的 `sort` 稀疏步长约定**（重排写 `(index+1)*100`），不用 folders 的 `createdAt` 排序：分组顺序即用户端分区顺序，需要管理员显式拖拽控制。`models.group_id` → `model_groups.id`（`ON DELETE SET NULL`；⚠ drizzle-kit 生成的 `ADD COLUMN ... REFERENCES` 会丢 `ON DELETE`，0024 迁移已手工补上，与 0013 同一个坑），索引 `models_group_idx`。`services/model-groups.ts` 删除分组时**不依赖 FK**，而是在事务里逐行把 `group_id` 置 null 并**写回各行原 `updatedAt`**——分组归属变化不该让模型看起来"刚被改过"（与 `deleteFolder` 处理会话同理）。
...
- `folders`：**聊天文件夹**（每用户私有）——`name`/`color`(#RRGGBB，旧记录的 null 在读取时归一为默认黄色)/`emoji`(图标，null=默认文件夹图形)/`pinned_at`。`services/folders.ts` + 路由 `routes/folders.ts`；删除文件夹时其中会话移回未分组（FK `ON DELETE SET NULL` + 服务层显式移出并逐行保留 `updatedAt`）。
...
- `runEngine`（`engine.ts`，文本）：`persistEmit(type,data)` 先用纯拷贝净化图片结果与 reasoning 密文，再**写 `run_events`（同步 better-sqlite3）并 `runEmitter.emit`**，自增 `seq`；原始对象仍留在内存供终态处理。先发合成 `run.created`，置 run `running`；然后 `for await` 消费 `client.createResponseStream(body, signal)`。delta 只负责低延迟累积：分别接收 `response.reasoning_summary_text.delta` 与 `response.reasoning_text.delta`，摘要存在时优先、仅有明文推理时回落展示 raw 文本；`response.completed/incomplete` 的完整 `response.output` 必须用于最终正文、思考摘要、引用、usage、状态校准，并在开关/effort 门控通过时从原始终态提取 reasoning items（单轮 JSON >256KB 整轮放弃并告警）形成私有重放信封；没有任何终态事件就 EOF 视为失败，不能把 delta 当最终真值。建流 4xx 可按原始 type/code/message 识别“不支持 include”或“历史 reasoning 无效”，仅在首个上游事件前去掉对应字段重试一次，流开始后绝不重试。中止（signal.aborted）→ state `canceled`。
...
- `params.ts` `buildResponseBody`：参数优先级 **高级硬参数 > 应用生成参数 > 用户请求 > 模型默认 > 代码默认**。思考按 `allowed_efforts` 校验后才发 `reasoning.effort`；历史推理开关 + 有效非 `none` effort 时加 `include:['reasoning.encrypted_content']`，`include` 与高级参数按去重并集合并（关开关不删除管理员手写 include）；联网仅 `capabilities.web_search && 开关` 才挂 `tools:[{type:'web_search'}]`，X 搜索同理挂 `{type:'x_search'}`（两者独立、可同时下发）；**`tools` 数组按工具身份（`responseToolMergeKey`）合并而不是整体替换**：命中已有条目时与应用生成的配置浅合并（管理员只写想改的字段），未命中时追加——但 `web_search`/`x_search` 这类**启停由开关决定的工具例外，未命中即整条丢弃**，因此高级 JSON 里的 `{"tools":[{"type":"web_search","enable_image_search":false}]}` 只是「参数模板」：开关开时生效、关时既不注入工具也不留下空 `tools` 字段（⚠ 早期实现里它会把工具塞回请求，令会话开关失效）。要让某模型恒开检索，改能力位 + `default_web_search`，不要靠高级 JSON 注入；`tools: []` 仍保留「清空全部工具」的兜底语义；开思考时 `max_output_tokens` 兜底 ≥25k（`REASONING_MIN_OUTPUT_TOKENS`）。所有文本请求写入会话级稳定 `prompt_cache_key`；最后 `mergeDeep(hardParams)`，因此高级 JSON 可显式覆盖该 key 并原样传递其他上游参数。Chat Completions 使用独立 builder，仅复用高级参数最终覆盖与稳定 `prompt_cache_key`；站内 Web/X Search 能力对 Chat 恒为关闭。`buildImageBody`/`buildImageEditBody` 只组装 size/quality/background 与参考图；`gpt-image-2` 尺寸规则在 `prepare.ts` 调共享 util 校验，避免把 provider fetch 层写成业务校验层。
...
- **账号菜单**（`Sidebar.tsx#AccountMenu`）：侧栏底部账号行与折叠栏头像共用同一个弹层；顶部展示放大的真实头像（无头像时使用首字母渐变兜底）、昵称与角色徽章，设置/管理后台为常规操作区，两项之间保留 2px 纵向间距；退出登录由独立分隔线与常驻红色危险语义区分。操作文字固定为整数 14px / 20px 行高，图形统一为整数 16px，并置于与文字行高同高的 20px 固定槽位中，避免浏览器缩放时半像素字号导致的栅格化错位。触发按钮以 `aria-expanded` + `aria-controls` 暴露开合状态，菜单项保留清晰的键盘焦点环；浅色/深色各自使用匹配的边框、表面、徽章与悬停色。
- **侧边栏文件夹 + 批量管理（里程碑 E）**：
  ...
  - **文件夹设置弹窗**（`FolderEditorDialog.tsx`，创建/编辑两用，全局挂载由 `store/folderEditor.ts` 驱动）：`FolderIdentityField.tsx` 让 44px 高的名称输入与图标触发器并排；触发器内仍是 24px 无底板裸图标，仅用同样无底板的 ChevronDown 表达展开状态，不再向 Emoji 叠加装饰角标，图形尺寸与点击热区各自独立。桌面端新建时自动聚焦名称、展开 Emoji 时自动聚焦搜索，移动端两处均不抢焦点（避免连续唤起软键盘）；Emoji 面板 = **frimousse**（headless，`EmojiPickerPanel.tsx` 懒加载、Tailwind 全自绘、`locale=zh` 中文搜索、数据走同源 `/api/emoji-data`——服务端自托管 Emojibase，不依赖公网 CDN），搜索框与「选择图标」标题共用紧凑工具栏、不再独占一行；颜色 = 默认选中柔和黄色的 12 色浅色色板（与模型分组共用）+ 自定义（入口使用同系粉彩渐变；**react-colorful** HexColorPicker/HexColorInput，`.hc-color-picker` 覆写样式）；不再提供额外的灰色默认项；面板均内联展开，Emoji 列表按弹窗剩余高度自适应并独立滚动（浏览器放大时不产生弹窗外层滚动条），Escape 先收面板再关弹窗。
...
- `ModelControlMenu`：**聚合选择器**（取代旧 ModelSelector + ChatControls）——一个触发器（模型名 + 思考深度短标签 + 联网状态小地球 + 箭头）聚合全部会话参数，桌面/移动共用同一套分区组件（`MenuSections`）：
  ...
  - **列对齐约定**（`ModelListViews.tsx` 顶部注释 + `TRAILING_SLOT_CLASS` / `OVERLAY_INFO_BUTTON_CLASS` 两个常量，对标管理端模型页的 `ROW_INSET_X` 系列度量）：模型行与分组成员统一 `pl-3`，不再用缩进表达层级；有图标的真实分组与模型共用左侧图标列，「未分组」与显式 `none` 分组则直接从左侧内边距起排标题，不渲染透明占位。右侧统一 28px 操作列，ⓘ / 折叠箭头 / 钻取箭头共用同一中心线；有描述的模型把选中勾放在 ⓘ 左侧并为未选中态保留位置，无描述的模型则让选中勾直接复用最右侧 ⓘ 槽位。ⓘ 绝对定位覆盖在行上方（而非与行按钮并列），整行仍可点，`group-hover` 保证指针移到 ⓘ 上时行底色不闪断。相邻分区仅保留 4px 轻间隔；折叠箭头与组内内容以 200ms 同步过渡，内容用 CSS Grid `0fr ⇄ 1fr` 按真实高度展开，并在折叠态通过 `inert` 移出键盘交互。动画裁切使用 `overflow-clip` 而非 `overflow-hidden`，否则 Grid item 会丢失横向固有宽度贡献，令 `w-fit` 面板错误收窄并截断本可完整显示的模型名。
  - **两种列表视图（里程碑 J）**：列表本体与分组逻辑已从 `ModelControlMenu.tsx` 拆到 `ModelListViews.tsx` + `modelGroups.ts`（纯逻辑 + 单测，对标 `sidebarSections.ts`），弹层外壳只留触发器、定位与参数分区。二级目录停留在分组根层时只展示分组，已选文本模型的思考/检索参数与画图模型的分辨率/画质均统一隐藏；进入具体分组或搜索直接显示模型结果后才恢复参数区，避免参数在视觉上误归属于某个分组。
    ...
    - **二级目录视图**：一级为分组行（无底板裸图标 + 名称 + 数量 + `ChevronRight`），点进为二级列表带「← 分组名」返回头；进入下一级时内容从右侧轻推入，返回上一级时从左侧轻推入，方向与层级关系一致，并在 `prefers-reduced-motion` 下关闭。根层不展示参数区时按实际分组数量自然收高，不沿用参数区存在时为模型列表保留的最小高度。挂载时若当前选中模型在某组内则直接进入该组（延续原先 `scrollIntoView` 让选中项可见的意图）。
    - **搜索**：模型数 >8 时才出现（少量模型时搜索入口纯属占地方），默认只显示与视图切换同高的小图标，点击后才在原工具栏内展开完整输入框；输入框使用轻量浅填充，聚焦主要靠底色变化，仅保留极淡的 inset focus ring，避免描边压过模型列表。尾部按钮有内容时清空、无内容时收起，Escape / 空值失焦同样收起。有输入时**两种视图都退化为扁平结果**——此时用户找的是具体模型而非结构；命中分组名会保留该组全部模型（输入「Claude」期望看到整个 Claude 分组）。
    - **视图切换**：两个图标按钮（List / Layers3）挂在「模型」分区标题右侧，仅在存在真实分组时渲染。选择存进账户级偏好 `modelPickerView`（服务端为真值，与主题/字号同一套）；分组折叠状态存 `store/modelPicker.ts`（localStorage，照抄 `store/sidebar.ts` 的 `expandedFolders`）。
...
三种资源来源收敛为 `ModelIconAsset = {type:'lobe',slug} | {type:'custom',id} | {type:'emoji',char}`（`shared/types/domain.ts`）。模型字段使用 `ModelIcon = ModelIconAsset | {type:'initial'}`：`null` 表示未配置并允许自动识别，`initial` 表示显式强制名称首字母；分组字段使用 `ModelGroupIcon = ModelIconAsset | {type:'none'}`：`null` 保持默认文件夹图形，`none` 表示真正不渲染图标与占位。模型与分组的专用模式互不接受。

- **归一化**（`shared/util/modelIcon.ts`）：`normalizeModelIconAsset(value: unknown)` 只接受三种资源，`normalizeModelIcon(value: unknown)` 额外接受模型专用 `initial`，`normalizeModelGroupIcon(value: unknown)` 则额外接受分组专用 `none`；三者与 `normalizeModelTags` 契约一致——入参 `unknown`、绝不抛错、非法值静默降级为 `null`。slug 用 `LOBE_ICON_SLUG_PATTERN = /^[a-z0-9][a-z0-9-]{0,63}$/` 严格限制（**不允许点号**，挡住 `..` 穿越；不允许斜杠与标点，挡住 CSS/URL 注入），因为它会被拼进 `<img src>` 与 CSS `mask-image: url(...)`。Emoji 复用文件夹的单字素簇判定。zod 侧 `shared/schemas/model-group.ts` 以 `modelIconAssetSchema` 为公共资源口径，另设 `modelIconSchema` / `modelGroupIconSchema` 隔离两个专用模式；两端都在 DTO 边界调用（`toModelDTO`/`toModelGroupDTO`）。
- **自动识别**（`shared/util/modelIconGuess.ts`）：`guessModelIconSlug(modelId, displayName?)` 按有序规则表（先具体后宽泛；GPT Image 明确映射 OpenAI 通用图标，避免误套 DALL·E 品牌）匹配约 54 个品牌，认不出返回 `null`（宁可漏判用首字母兜底，不可错判）。**仅 `icon=null` 时前端在渲染层回退到这个猜测**；显式 `{type:'initial'}` 会绕过猜测，即使管理员填写了可识别的模型 ID 也始终显示名称首字母。管理端「批量识别图标」只是把同一份猜测**固化成可编辑的显式值**。空白的新建草稿不渲染伪首字母或占位点。单测里有一条防回归断言：规则表引用的每个 slug 都必须真实存在于已安装的图标包（写错 slug 只会静默显示首字母，不报错）。
...
- `ModelsPage.tsx`：卡片列表 + **dnd-kit 拖拽排序**（`@dnd-kit/core+sortable`，键盘可用；搜索/供应商筛选生效时禁用拖拽），行内直接显示图标（`components/ModelIcon.tsx`）、标签徽章（`components/ModelTags.tsx`）与能力徽章；列表开关是唯一全局上下架入口，另有“全部用户 / 指定 N 人”范围按钮打开 `ModelAccessDialog.tsx`。范围面板按管理员/普通用户分组，支持名称/用户名搜索、搜索结果全选、分组三态选择、停用账号预授权及浅色/深色/移动端布局；每次打开都使用新的编辑快照，保存前会检测并发修改或已删除账号，不会静默覆盖旧名单。
  - **「分组显示」开关按钮**：与「批量管理」同规格的开关式按钮（`aria-pressed` 表达开关态），开启后按分组分区渲染（空分组也显示，管理员要能看见），并**禁用拖拽**——沿用「筛选中不可拖拽排序」的既有先例（分组视图重排了行的位置，全局顺序无法可靠映射）。这样 `reorderModels`「必须提交穷尽的全局 id 列表」的契约完全不用动。排序提示语单独占一行，不参与工具栏的 flex 排布，避免它出现/消失时挤压搜索框。分组标题、空分组占位与模型行共用 `ROW_INSET_X / ROW_GAP_X / ROW_ICON_COLUMN` 三个度量常量，保证左侧基线对齐。
  - **「批量管理」模式**（`ModelBatchTools.tsx`）：行首多选框、整行点击切换选中（选中行 sky 底色）、行内操作按钮隐藏；底部为一条居中的**不透明胶囊悬浮条**（`sticky bottom-4`，单行：三态全选框 + `已选 N / M` + 两个批量操作 + 「完成」；不透明底色避免透出下方列表行），操作为「移动到分组」（`AssignGroupDialog`，列出全部分组 + 移出分组，点选即执行）与「批量识别图标」（`BatchIconDialog`，先展示 `现图标 → 识别结果` 的差异列表再确认；**默认只处理未设置图标的模型**，勾选后才覆盖管理员手工挑过的图标；识别不出来时不会把已有图标抹成空）。两者都走原子批量接口，任一 id 不存在则整批失败并回报 `invalidIds`，不留「移了一半」的中间态。
- **`ModelGroupsPage.tsx` + `ModelGroupEditor.tsx`（新页，导航「接入 → 模型分组」）**：dnd-kit 拖拽排序列表（sensors / `SortableContext` / 乐观更新三件套照抄 `ModelsPage`），行内 `ModelGroupGlyph` + 名称 + 模型数 + 编辑/删除；删除走 `askConfirm` 并明确提示「组内 N 个模型会移到未分组，模型本身不会被删除」。编辑弹窗按「名称 → 图标 → 颜色」排列，不再额外占一行重复预览；默认文件夹或模型自动识别的实际回退图形直接嵌进 `IconPicker` 摘要字段，仍与用户端复用同一套无底板图标组件。分组可在摘要栏一键切到“无图标”，展开选择器后该选项固定在来源分页之外，切到自定义 / Emoji 页仍可达；它以稳定标签的 `aria-pressed` 按钮表达选中态。默认文件夹颜色默认选中柔和黄色，与聊天文件夹共用 `FOLDER_COLOR_PRESETS` 浅色预设和 react-colorful 自定义入口。显式资源图标和无图标模式都与颜色互斥：选择后即清空并隐藏颜色字段；恢复默认文件夹图形时使用默认黄色；色板的 28px 无边框圆点共用 `components/ui/ColorSwatch.tsx`，避免 `conic-gradient` 与半透明边框再次产生四边串色。
- **`components/IconPicker.tsx`（管理端共用）**：折叠态是完整的摘要字段，显示当前有效图形、来源说明和展开箭头；图形直接落在摘要行中，不再在小图标外重复套一层装饰底板。选中显式图标后提供独立且有足够热区的「恢复默认」，模型自动识别成功时则提供「使用首字母」捷径，并可随时恢复自动识别。展开态有三个分页（内置 / 自定义 / Emoji），内置品牌搜索与 Emoji 中文搜索都放在分页右侧，窄屏自动换到下一行；`iconSearch.ts` 以品牌前缀索引支持大小写不敏感的中文品牌名、全拼与英文 slug（含「ChatGPT → openai」「千问 / qianwen / qwen」等别名），不向管理端加载整套运行时拼音字典。选择直接写入宿主表单草稿，最后仍由弹窗「保存」统一提交，网格选中态只表达当前值，不再额外放一个含义不明的确认勾号，也不再用“重复点击选中项”暗中清除。Escape 先收起选择器并把焦点还给摘要字段。⚠ 内置库有 903 个图标，**一次性渲染会同时发出 903 个 SVG 请求**：默认只展示 `curatedIcons.ts` 里策展的约 80 个高频品牌，输入搜索才从完整 catalog 过滤且最多渲染 120 个（`ICON_SEARCH_RESULT_LIMIT`），把并发图片请求钉死在可控量级；网格采用独立的紧凑滚动区，避免把颜色与底部保存操作推到弹窗视口外。Emoji 分页继续懒加载并复用聊天端的 `EmojiPickerPanel`。服务端用独立详情接口按需返回完整名单，列表只返回人数。
...
13. **协议能力边界**：`model.kind='chat'` 支持文本/`reasoning_content`/图片及官方 Chat 文件内容块，但不提供站内 Web/X Search 能力；旧 Chat 模型记录中的搜索能力、默认值与会话参数会在 API/运行边界归一为关闭。Anthropic 支持自己的原生 web search 和 URL citation；X 搜索仍只属于 Responses/xAI。普通客户端工具调用尚无本地执行器，会以 failed 明确终结并向用户展示原因。
...
| 用户设置/偏好                                                          | `web/src/store/settings.ts`、`web/src/chat/SettingsDialog.tsx`（偏好键与默认值在 `shared/util/preferences.ts` + `shared/schemas/settings.ts`；分页：通用/消息显示/账户/我的分享/关于；各页统一使用页级标题与 `SettingsSection` 语义分区）                                                                                                                |
...
| 模型 / 分组图标（三类资源、自动识别、显式首字母、品牌搜索、mask 渲染） | `shared/types/domain.ts`（`ModelIconAsset` / `ModelIcon`）、`shared/util/modelIcon.ts` + `modelIconGuess.ts`、`server/services/lobe-icons.ts`、`server/routes/model-icons.ts`、`web/src/components/ModelIcon.tsx` + `iconSizing.ts` + `IconPicker.tsx` + `iconSearch.ts` + `curatedIcons.ts`、`web/src/index.css` 的 `.hc-icon-mask`/`.hc-colored-glyph` |
...
| 选择器面板尺寸过渡动画                                                 | `web/src/hooks/useSizeTransition.ts`                                                                                                                                                                                                                                                                                                                     |
...
| 文件夹设置弹窗（Emoji/颜色）                                           | `web/src/chat/FolderEditorDialog.tsx`、`EmojiPickerPanel.tsx`（frimousse）、`web/src/components/colorPresets.ts`、`web/src/store/folderEditor.ts`、`server/routes/emoji-data.ts`（自托管 Emojibase）                                                                                                                                                     |
...
| 上游事件解析/去 obfuscation / 思考摘要 part 边界（含 raw reasoning）   | `server/provider/sse-parse.ts`、`shared/util/reasoningSummary.ts`                                                                                                                                                                                                                                                                                        |
