...
- **后台自动重试**：管理员可在「系统设置 → 自动重试」开启（默认关闭），配置次数、递增间隔、随机延迟、首次输出等待、输出停滞等待和总时间上限。覆盖连接失败、HTTP 临时错误、响应中的过载/限流错误和断流，区分“尚未输出”与“输出中断”。可单独关闭输出中断后的自动重新生成；开启时等待期间保留旧内容，新回答开始后整体替换。聊天显示阶段、次数和倒计时，离开页面或刷新不影响服务端继续，随时可停止。请求事件与错误日志保留重试经过和恢复结果，删除聊天后仍可查；用量累计上游已报告的各次尝试，重试可能产生额外费用。密钥、参数、余额、拒绝和内容过滤不重试；服务重启仍会中断未完成任务。
