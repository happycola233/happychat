...
- **近期里程碑 A–D（本文档已据此更新）**：
  ...
  - **F 独立分支对话**：助手消息操作区可把“根消息 → 该助手消息”的路径复制为新的独立会话，标题为 `分支 • 原对话名称`；附件使用新 ID 与独立磁盘副本，删除原对话或分支互不影响；不复制 run/event/usage 审计记录，避免后台统计失真。
- 验证方式：`scripts/` 下脚本（Playwright 浏览器 E2E + 直连后端冒烟）+ 临时库 curl 冒烟，全部对真实 OpenAI 兼容上游（Base URL/Key 在管理后台 Provider 中配置，不写进代码）跑通。
- `npm run typecheck` / `lint` / `test`（56 个单测文件、350 用例）/ `build` 全绿。
...
- **独立分支对话**：`POST /api/conversations/:id/branch {assistantMessageId}` 只复制该消息的连续祖先链，所有 message/attachment ID 与 parentId/attachment_id 都重写，`activeLeafId` 指向新助手消息；继承原文件夹但不继承置顶/分享。历史 `run_id` 清空，目标 run 的用户可调参数快照写入新会话 `params_override`，`getConversationLastRun` 在无 run 时回退到该快照，保证刷新后仍恢复正确模型/联网/思考设置。附件先复制文件，再以单个 SQLite 事务写会话、消息和附件行；文件或事务失败会清理本次新文件。
...
- **删除清理**：删除会话（单条 `DELETE /:id` 与批量 `POST /batch-delete` 共用 `deleteConversations`）会一并清理其消息关联的附件行与磁盘文件（此前单条删除会遗留孤儿附件，里程碑 E 顺手修复）；「清空全部对话」仍删除该用户全部附件。
...
- `ModelEditor.tsx` **新建/编辑两用**：模型 ID、外显名称、**描述（用户可见，选择器 ⓘ）**、**标签（`TagsInput.tsx` 芯片输入，与用户端同配色所见即所得）**、能力（两列开关）、思考等级（`ReasoningEffortEditor.tsx`：**单容器行内编辑列表 + 拖拽排序 + 行内「设默认」标记**，幽灵输入框；不再有单独的默认下拉框）、默认参数、缓存策略、**定价（USD/1M，分别配置普通输入、缓存写入、「缓存读取/输入」、输出）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON）**。
...
- 单测（`npm run test`）：覆盖 URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、run 准备、独立分支的路径截断/权限/父链/参数恢复/附件独立生命周期与失败回滚、前端分支按钮/原始图标/持久化流式态门控、流式 reducer 与推理摘要 Markdown（含多 reasoning item/summary part、续传压缩、终态校准及历史相邻粗体兜底）、Markdown CJK 邻接强调/安全性（危险 HTML/协议过滤——公告正文复用该渲染器）与 GitHub Alerts 渲染。
