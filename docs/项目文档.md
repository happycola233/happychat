...
- `providers`：`api_key` 明文保存，列表固定脱敏、编辑详情按权限返回完整 Key；`prompt_cache_retention` 为该上游的保留策略（null=上游默认，`24h`=扩展缓存）。`models`：`prompt_cache_enabled` 决定该文本模型是否应用 Provider 的保留策略；`capabilities`/`default_params`/`hard_params`/`allowed_efforts`/`pricing` 都是 JSON；其中 `allowed_efforts` 新记录为有序 `{value,description}[]`，读取时兼容旧 `string[]`，无需 DDL；`kind` ∈ `responses|chat|image`（`chat`=走 chat/completions）；`pricing`={input,cacheWriteInput,cachedInput,output,image}（USD/1M，缓存写入/读取分别定价，用于成本估算）；`UNIQUE(provider_id, model_id)`。
...
- `ModelEditor.tsx` 现为**新建/编辑两用**：可设模型 ID 与外显名称、能力、思考等级（有序编辑上游 value + 中文 description，支持增删/上下移动/默认项）、默认参数、是否应用 Provider 缓存保留策略、**定价（USD/1M，普通输入/缓存写入/缓存读取/输出分别配置）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON，最终优先、去黑盒）**。
