...
const lobeIconSchema = z.object({
  type: z.literal('lobe'),
  slug: z.string().trim().toLowerCase().regex(LOBE_ICON_SLUG_PATTERN, '内置图标标识不合法'),
})
...
/** 模型与分组都能使用的三种真实图形资源。 */
export const modelIconAssetSchema = z.discriminatedUnion('type', [
  lobeIconSchema,
  customIconSchema,
  emojiIconSchema,
])
