...
- `ReasoningCard`：组件内计时器（每 200ms）+ 思考中自动展开/完成自动折叠 + 自动滚底。⚠ 状态行与摘要体之间的间距放在可折叠容器内部（`pt-2`）而**不是根上的 space-y**：Tailwind v4 的 space-y 给前一个兄弟加 margin-bottom，折叠后 0 高度容器会留下死空间，导致「已思考」与下方搜索状态行折叠时间距偏大（已按用户反馈修正，勿改回）。
