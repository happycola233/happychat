...
- `npm run typecheck` / `lint` / `test`(32 个单测文件、162 用例) / `build` 全绿。
...
- `Markdown`：`react-markdown` + `remark-gfm`/`remark-math` + `rehype-katex`/`rehype-highlight`（流式时按 `animate` 再挂 `rehypeStreamFade`：把正文按可见单元包 `<span class="hc-stream-seg">`，靠 CSS 让新到达文字逐段渐入，替代打字光标，代码/公式子树跳过），**不开 `rehype-raw`**（故原始 HTML 当文本转义，安全）；`PreBlock` 给代码块加复制按钮（读 `textContent`）；表格包横向滚动；`memo` 化避免流式每 token 全量重渲。
- `ReasoningCard`：组件内计时器（每 200ms）+ 思考中自动展开/完成自动折叠 + 自动滚底。
- `ChatControls`：按 `model.kind` 与 `capabilities` 分派——文本模型显示联网开关 + 思考等级；图片模型显示尺寸/质量。尺寸菜单来自 `shared/util/imageSize.ts`，包含 `auto`、常用预设分辨率与自定义宽高；自定义按 OpenAI `gpt-image-2` 约束即时校验。
...
- 单测（`npm run test`）：覆盖 URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、run 准备、前端流式 reducer 与推理摘要 Markdown、Markdown 安全性（原始 HTML 惰性化、`javascript:` 中和——公告正文复用该渲染器）。
...
| 用户设置/偏好 | `web/src/store/settings.ts`、`web/src/chat/SettingsDialog.tsx` |
| 编排器临时vs固定 + 按会话恢复 | `web/src/store/chat.ts`、`web/src/chat/ChatView.tsx`、`ChatControls.tsx` |
...
| Markdown/代码/公式/流式渐入 | `web/src/chat/Markdown.tsx`（渐入插件 `web/src/chat/markdownStreamFade.ts`）+ `web/src/index.css` 的 `.hc-md`/`.hc-stream-seg` |
