...
- `npm run typecheck` / `lint` / `test`（49 个单测文件、296 用例）/ `build` 全绿。
...
- 单测（`npm run test`）：覆盖 URL、Key 脱敏、SSE 解析、Provider 参数、模型默认能力、run 准备、前端流式 reducer 与推理摘要 Markdown（含多 reasoning item/summary part、续传压缩、终态校准及历史相邻粗体兜底）、Markdown 安全性（危险 HTML/协议过滤——公告正文复用该渲染器）与 GitHub Alerts 渲染。
