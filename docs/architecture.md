...
- **账号菜单**（`Sidebar.tsx#AccountMenu`）：侧栏底部账号行与折叠栏头像共用同一个弹层；顶部展示放大的真实头像（无头像时使用首字母渐变兜底）、昵称与角色徽章，设置/管理后台为常规操作区，退出登录由独立分隔线与常驻红色危险语义区分。操作图形统一为 16px，并置于与 20px 文字行高同高的固定槽位中，保证不同 SVG 轮廓与文字视觉居中。触发按钮以 `aria-expanded` + `aria-controls` 暴露开合状态，菜单项保留清晰的键盘焦点环；浅色/深色各自使用匹配的边框、表面、徽章与悬停色。
- **侧边栏文件夹 + 批量管理（里程碑 E）**：
