...
- **用户设置中心**：主题、重点色、字号、Enter 发送（桌面 / 手机分别配置）、自动滚动、消息时间 / 模型名 / Token（含缓存写入读取）· TPS · 耗时明细开关；管理员还可全局决定是否在用量行末尾展示本次预估成本（USD，成本为 0 时不显示）
...
| **系统设置**            | “注册需要邀请码”、“允许用户分享聊天”、“展示单次请求成本”开关，以及标题总结模型与提示词（默认已填好，可直接改）等全局配置                                                                     |
