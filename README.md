...
- **模型 / 分组图标**：内置 900+ AI 品牌图标（[lobe-icons](https://github.com/lobehub/lobe-icons)，**随应用自托管，不依赖公网 CDN**），也可上传自定义图标或直接用 Emoji；未配置时按模型 ID **自动识别品牌图标**，管理端还能批量识别并套用。单色图标经 CSS mask 渲染，浅色 / 深色下均自动适配
