...
- **周期与重置**：自然日 / 周 / 月自动重置（边界时区与周起始日可配），也支持 Codex、Claude Code 风格的滚动窗口（5 小时 / 7 天 / 30 天 / 自定义）与永久累计
- **真正的无限额度**：不用一个很大的数字模拟——界面显式显示「无限额度」
- **按模型精细拦截**：某个模型额度耗尽只禁用该模型，选择器里标记「额度已用尽」，其他仍有额度的模型照常可用
- **临时增加额度**：给某人本周期临时加 $5（$10 → $15），周期结束自动失效，不污染长期配置；也可手动重置当前周期（只抬高统计起点，历史用量与后台统计不受影响）
- **暂停 / 恢复限额**：临时放行某个用户，期间用量照常累计，恢复后立即按累计值重新判定
- **额度预警**：接近上限或已耗尽时在输入框上方给出提示条，说明用了多少、何时重置、能否换模型
- **个人使用情况面板**（`/usage`）：GitHub 贡献图风格的活跃热力图（可切换请求数 / Token / 花费）、连续活跃天数、最常用模型、按模型的用量与花费构成、一天与一周中的活跃分布，以及自己的额度进度
...
| **用户限额**            | 策略模板 CRUD / 复制 / 设为默认 / 拖拽排序；用户视图查看每人策略与用量进度、批量修改策略、暂停或恢复限额、手动重置周期、临时赠送额度、逐规则覆写并在保存前**预览最终生效结果**                 |
