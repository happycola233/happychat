...
- `ModelControlMenu`：**聚合选择器**（取代旧 ModelSelector + ChatControls）——一个触发器（模型名 + 思考深度短标签 + 联网状态小地球 + 箭头）聚合全部会话参数，桌面/移动共用同一套分区组件（`MenuSections`）：
  ...
  - **桌面端**为锚定弹层（输入框内向上、宽 w-80；首选方向可用高度不足 <420px 且对侧更宽裕时自动翻转——新对话输入框居中时即向下弹，并按所选方向动态限高，缩放动画原点随方向切换）；**移动端**为底部弹层 bottom sheet（`createPortal` 到 body，遮罩点击/Escape 关闭，顶部抓手、安全区内边距、更大触控行高，动效 `hc-sheet-in` 克制快出）。面板内部为弹性布局——思考/联网/图片参数分区始终可见，仅模型列表在空间不足时收缩滚动（最低保留约 3.5 行）。
