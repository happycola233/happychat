...
- `ModelEditor.tsx` **新建/编辑两用**：模型 ID、外显名称、**描述（用户可见，选择器 ⓘ）**、**标签（`TagsInput.tsx` 芯片输入，与用户端同配色所见即所得）**、能力（两列开关）、思考等级（`ReasoningEffortEditor.tsx`：仅在思考能力开启时显示，关闭后隐藏但保留配置；**单容器行内编辑列表 + 拖拽排序 + 行内「设默认」标记**，幽灵输入框；手动新建时预填 `none/low/medium/high/xhigh` 五档但不擅自设置默认档位，不再有单独的默认下拉框）；Responses + reasoning 时同区显示「保留历史推理上下文」开关，默认关闭。另含默认参数、**定价（USD/1M，分别配置普通输入、缓存写入、「缓存读取/输入」、输出）**、系统提示词（带变量图例）、**请求体硬参数（原始 JSON）**。重复的“在用户端启用”开关已移除，避免旧表单覆盖列表里的最新全局状态。
